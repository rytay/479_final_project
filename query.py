...
def user_query(index, doc_lengths , url_dict , ai_df, top:int ,ranker='tf_idf', k=1.5, b=0.5,use_ai_terms=False):
    ...
    q = []
    query_string = ''
    prompt = input('Enter "no" to stop query, otherwise press any key to continue: ')
    if prompt == 'no':
...
def query_ai(index, ai_df, doc_lengths , url_dict , top:int ,ranker='tf_idf', k=1.5, b=0.5,augment=None,result_dir=os.getcwd()+'/results/'):
    ...
    if prompt == 'no':
        ...
        if len(to_process) != 0:
            ...
            #Create final result list, sort in decreasing order and display/log result
            ranked.sort(key=lambda x: x[0], reverse=True)
            log = "==Using AIindex augmentation=="
            log += 'Results for Query: \'' + query_string +'\'\n'
            log +='('+ranker+', url)\n'
            log += ('==================\n')
            if top > len(ranked) - 1:
            ...
            with open(result_dir+'AI_top_'+str(top)+'_ranker_'+ranker+'_k='+str(k)+'_b='+'_'+str(b)+'_'+str(datetime.now())+'_'+query_string+'.txt','w+') as f:
                f.write(log)
            print(log)
...
def query_auto_ai(index, ai_df, doc_lengths , url_dict , top:int ,ranker='tf_idf', k=1.5, b=0.5,augment=None,result_dir=os.getcwd()+'/results/',query_string=''):
    q = tokenize(query_string)
    if augment is not None:
            q.extend(augment)
    print(q)
...
def query_auto(index, ai_df, doc_lengths , url_dict , top:int ,ranker='tf_idf', k=1.5, b=0.5,augment=None,result_dir=os.getcwd()+'/results/',query_string='',use_ai_terms=False):
    #Checking if results directory exists, otherwise create it
    if 'results' not in os.listdir(os.getcwd()):
        os.mkdir('results')
    prefix = ''
